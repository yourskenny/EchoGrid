...
if (collected < required) {
  ...
  if (pathToArtifact && pathToArtifact.length > 1) {
    console.log(moveTo(position, pathToArtifact[1]));
    process.exit(0);
  }
}
...
if (adjacentUnknown.length > 0) {
  ...
  console.log(`probe ${target[0]} ${target[1]}`);
  process.exit(0);
}
...
if (remoteUnknown) {
  console.log(`probe ${remoteUnknown[0]} ${remoteUnknown[1]}`);
  process.exit(0);
}
