...
function main(argv = process.argv.slice(2)) {
  ...
  const artifactPaths = {
    ...
    protocol_trace: resolvePath(cwd, options['protocol-trace'] || path.join(path.dirname(source), 'PROTOCOL_TRACE.md')),
...
function usage() {
  return `Usage:
  node ./scripts/write-demo-manifest.js <log.jsonl> [--out MANIFEST.json] [--index index.html] [--dashboard-html mission-control.html] [--scorecard SCORECARD.md] [--brief JUDGE_BRIEF.md] [--protocol-trace PROTOCOL_TRACE.md] [--replay-html replay.html] [--arena-html arena.html] [--leaderboard leaderboard.md] [--comparison-json agent-comparison.json] [--comparison agent-comparison.txt]
