...
- `action_hints.goal`: public target used to rank hints; follows high heat or artifact trace before artifacts are collected, then exit.
...
For audited rule-experiment policies, see `agents/rule-aware.js` and `agents/rule-explorer.js`. The showcase `rule-aware` agent checks bounded sector and row probes, claims only when a public `rule_signal` appears, then delegates routing to the baseline policy.
